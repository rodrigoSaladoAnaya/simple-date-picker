...
class SDPWeek extends LitElement {
  ...
  getClass(current) {
    let clazz = '';
    const date = this.date.__sdp_parse();
    if(this.selected == 'undefined' && current.__sdp_format() == new Date().__sdp_format()) {
      clazz = 'is-today';
    } else if(this.selected != 'undefined' && this.selected == current.__sdp_format()) {
      clazz = 'is-selected';
  ...
  selectDay({target}) {
    const data = {
      bubbles: true,
      composed: true,
      detail: { selected: target.getAttribute('date') }
    };
  ...
  getDay(dayPos) {
    ...
    return html`
      <sdp-day
        ...
        @click=${(event) => this.selectDay(event)}>
        ${current.getDate()}
      </sdp-day>
    `;
  }
